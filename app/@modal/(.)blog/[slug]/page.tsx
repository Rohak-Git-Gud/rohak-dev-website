...
import { BlogModal } from "@/components/blog-helpers/blog-modal";
...
export default async function BlogModalPage({ params }: PageProps) {
	const { slug } = await params;
	const blog = getBlogPost(slug);
	...
	return (
		<BlogModal title={blog.title} description={blog.description}>
			<article>
				{/* Header */}
				<h1 className="text-xl md:text-3xl font-bold text-primary mb-3">{blog.title}</h1>
